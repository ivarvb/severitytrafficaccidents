...
def vis_confusion_matrix(cfm, cats, nme):
    ...
    ax.set_title('Confusion Matrix - '+nme)
    plt.savefig("./plots/"+nme+'.png', format='png', dpi=300, bbox_inches='tight')
...
def printresults(ml, method, y_test, rfc_pred_test, labelmaping):

    print(method)

    filename = "./models/"+method+'.h5'
    ...
    vis_confusion_matrix(cfm, labelmaping, method)
...
printresults(model, "GBC", y_test, y_pred_test, labelmaping)
...
printresults(rfc, "RFC", y_test, y_pred_test, labelmaping)
...
model.fit(X_train, y_train)
y_pred_test = model.predict(X_test)
printresults(model, "KNNC", y_test, y_pred_test, labelmaping)
...
printresults(clf, "SVMC", y_test, y_pred_test, labelmaping)
...
clf.fit(X_train, y_train)
y_pred_test = clf.predict(X_test)
printresults(clf, "MLPC", y_test, y_pred_test, labelmaping)
